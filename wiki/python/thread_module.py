...
NOTE: If you are doing something that is more IO bound, such as reading from many sockets in a network application, or calling out to subprocess, you can get performance increases from threads.

NOTE: If what you are doing is more CPU bound then multiprocessing is more useful. If you use multithreading in this scenario then it may even slow down the process.

Doing multiple calulations in threads is going to dramatically degrade performance. This is specific to Python and is due to the Global Interpreter Lock (GIL).
...
Multithreading; multiple threads (subprocesses) that exist within the context of a process such that they execute independently but share their process resources.

Because of the GIL, threads in Python does not execute code simultaneously,
instead they run concurrently.

Concurrency;  when two or more tasks can start, run, and complete in overlapping time periods. It doesn't necessarily mean they'll ever both be running at the same instant. For example, multitasking on a single-core machine.
